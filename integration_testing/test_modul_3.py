...
@pytest.fixture(scope="module")
# Login Reviewer Internal Handler
...
@pytest.fixture(scope="module")
# Login Reviewer External Handler
def access_token_reviewer_external():
    login_response = client.post(
        "/api/auth",
        data={
            "username": "reviewer",
...
def test_reviewer_external_login():
    login_response = client.post(
        "/api/auth",
        data={
            "username": "reviewer",
...
def test_get_fraud_assessments_history_from_reviewer_external(access_token_reviewer_external):
    response = client.get(
        "/api/assessments",
    ...
    # Filter assessments where id_reviewer_external is empty and hasil_internal is not null
    assessments = response.json()["data"]
    filtered_assessments = [
        assessment for assessment in assessments
        if assessment["id_reviewer_external"] == "" and assessment["hasil_internal"] is not None
    ]
    
    assert len(filtered_assessments) > 0, "No assessments with id_reviewer_external empty and hasil_internal not null found"

@pytest.mark.order(19)
def test_assessment_evaluation_failed(access_token_reviewer_external):
    response = client.get(
        "/api/assessments/evaluation",
        headers={"Authorization": f"Bearer {access_token_reviewer_external}"},
        params={
...
def test_assessment_evaluation_success(access_token_reviewer_external):
    response = client.get(
        "/api/assessments/evaluation",
        headers={"Authorization": f"Bearer {access_token_reviewer_external}"},
        params={
...
def test_do_assessment_evaluation_from_external_reviewer(access_token_reviewer_external):
    id_assessment = "4rt70j4v2swm"
    response = client.post(
        f"/api/assessments/evaluation",
        headers={"Authorization": f"Bearer {access_token_reviewer_external}"},
...
def test_submit_assessment_evaluation_from_external_review(access_token_reviewer_external):
    response = client.get(
        "/api/assessments/finish",
        headers={"Authorization": f"Bearer {access_token_reviewer_external}"},
        params={
...
def test_get_fraud_assessments_history_from_reviewer_external_after_submit(access_token_reviewer_external):
    response = client.get(
        "/api/assessments",
        headers={"Authorization": f"Bearer {access_token_reviewer_external}"},
    )
